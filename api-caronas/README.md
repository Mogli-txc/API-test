...
| GET    | `/buscar`             | JWT  | Busca com filtros: `?car_status=`, `?data=`, `?esc_id=`, `?cur_id=` |
| GET    | `/minhas`             | JWT  | Lista caronas do motorista autenticado (`?status=` opcional)   |
| GET    | `/passageiro`         | JWT  | Lista caronas onde o usuário é passageiro confirmado           |
...
Exige JWT + papel Admin (1) ou Desenvolvedor (2). Operações marcadas com **Dev** exigem `per_tipo = 2`.

#### Estatísticas

| Método | Rota                   | Acesso    | Descrição                                                      |
|--------|------------------------|-----------|----------------------------------------------------------------|
| GET    | `/stats/usuarios`      | Admin/Dev | Totais de usuários por status e verificação (inclui suspensos) |
...
| GET    | `/stats/documentos`    | Admin/Dev | Totais de documentos por tipo (comprovante/CNH) e status OCR   |
| GET    | `/stats/sistema`       | **Dev**   | Resumo consolidado de todos os módulos                         |
| GET    | `/stats/contratos`     | **Dev**   | Contratos de escolas: ativos, expirados, sem contrato, alertas de vencimento (90 dias) |

#### Gestão de usuários

| Método | Rota                                    | Acesso    | Descrição                                                                |
|--------|-----------------------------------------|-----------|--------------------------------------------------------------------------|
| POST   | `/cadastrar`                            | **Dev**   | Cria conta Admin/Dev sem OTP — login imediato com email+senha            |
| GET    | `/usuarios`                             | Admin/Dev | Lista usuários com busca (`?q=`) e cursor (`?cursor=`, `?esc_id=`)       |
| GET    | `/usuarios/:usu_id`                     | Admin/Dev | Dados completos de um usuário (Admin: só escola; Dev: qualquer)          |
| PUT    | `/usuarios/:usu_id/perfil`              | **Dev**   | Atualiza papel e escola do usuário — promoção a Admin                    |
| POST   | `/usuarios/:usu_id/redefinir-senha`     | **Dev**   | Redefine senha de conta Admin/Dev sem email, invalida sessões ativas     |
...
| POST   | `/usuarios/:usu_id/penalidades`         | Admin/Dev | Aplica penalidade a um usuário                                           |
...
| GET    | `/avaliacoes`    | Admin/Dev | Lista avaliações com nomes dos participantes (`?esc_id=` — escopo por avaliado)   |
...
| Método | Rota             | Acesso  | Descrição                                                                              |
|--------|------------------|---------|----------------------------------------------------------------------------------------|
| GET    | `/logs`          | **Dev** | Leitura do AUDIT_LOG com filtros (`?acao=`, `?tabela=`, `?usu_id=`)                   |
...
| Método | Rota                        | Acesso    | Descrição                                                      |
|--------|-----------------------------|-----------|----------------------------------------------------------------|
| GET    | `/escolas`                  | Admin/Dev | Lista escolas (Admin: só a própria; Dev: todas)                |
| GET    | `/escolas/:esc_id`          | Admin/Dev | Dados da escola com lista de cursos                            |
| POST   | `/escolas`                  | **Dev**   | Cria nova escola (geocodifica via Nominatim)                   |
| PUT    | `/escolas/:esc_id`          | **Dev**   | Atualiza dados de uma escola                                   |
| DELETE | `/escolas/:esc_id`          | **Dev**   | Remove escola sem cursos                                       |
| POST   | `/escolas/:esc_id/contrato` | **Dev**   | Define/renova contrato da escola (`duracao`: `1ano`/`2anos`/`5anos`) |
| DELETE | `/escolas/:esc_id/contrato` | **Dev**   | Cancela contrato da escola (zera campos para NULL)             |
| GET    | `/cursos`                   | Admin/Dev | Lista cursos com filtro por escola                             |
| POST   | `/escolas/:esc_id/cursos`   | **Dev**   | Cria curso em uma escola                                       |
| PUT    | `/cursos/:cur_id`           | **Dev**   | Atualiza dados de um curso                                     |
| DELETE | `/cursos/:cur_id`           | **Dev**   | Remove curso sem matrículas                                    |
...
Ações registradas: `LOGIN`, `LOGIN_FALHA`, `CADASTRO`, `OTP_FALHA`, `OTP_BLOQUEIO`, `SENHA_RESET`, `SENHA_RESET_ADMIN`, `DELETAR_USU`, `USU_ATIVAR`, `USU_INATIVAR`, `ADMIN_CADASTRAR`, `CARONA_CRIAR`, `CARONA_CANCEL`, `SOL_ACEITAR`, `SOL_RECUSAR`, `VEICULO_CADASTRAR`, `VEICULO_DESATIVAR`, `COMPROVANTE_APROVADO`, `CNH_APROVADA`, `PENALIDADE_APLICAR`, `PENALIDADE_SUSPENSAO`, `PENALIDADE_REMOVER`, `PERFIL_ATUALIZAR`, `ESCOLA_CRIAR`, `ESCOLA_ATUALIZAR`, `ESCOLA_DELETAR`, `CONTRATO_DEFINIR`, `CONTRATO_CANCELAR`, `CURSO_CRIAR`, `CURSO_ATUALIZAR`, `CURSO_DELETAR`.

#### `authHelper.js`

Centraliza verificações de autorização reutilizadas em múltiplos controllers:

- **`checkDevOrOwner(requesterId, targetId)`** — retorna `true` se o requester é o dono do recurso (IDs iguais) OU Desenvolvedor (`per_tipo = 2`).
- **`checkAdminOrOwner(requesterId, targetId)`** — como `checkDevOrOwner`, mas inclui Admin (`per_tipo >= 1`).
- **`getMotoristaId(caronaId)`** — retorna o `usu_id` do motorista de uma carona via JOIN. Retorna `null` se a carona não existir.
- **`isParticipanteCarona(caronaId, usuId)`** — retorna `true` (participante), `false` (não participante) ou `null` (carona não existe).
- **`verificarContratoEscola(usu_id, usu_email)`** — verifica se o contrato da escola do usuário está ativo. Admin (per_tipo=1): verifica pela `per_escola_id`. Usuário comum: verifica pelo domínio do email. Dev: nunca bloqueado. Retorna `{ bloqueado: boolean, mensagem? }`. Degrada graciosamente (`{ bloqueado: false }`) se a migration v11 ainda não foi aplicada ao banco (`ER_BAD_FIELD_ERROR`).
...
| Tabela | Propósito |
|---|---|
| `ESCOLAS` | Instituições de ensino cadastradas. `esc_dominio` (restrição de e-mail), `esc_max_usuarios` (cota). **[v11]** `esc_contrato_duracao` / `esc_contrato_inicio` / `esc_contrato_expira` — contrato com prazo (1/2/5 anos, gerenciado por Dev) |
...
### Implementações — **2026-04-28**

#### Novos endpoints — Gestão administrativa

| Endpoint | Acesso | Descrição |
|----------|--------|-----------|
| `POST /api/admin/cadastrar` | Dev | Cria conta Admin/Dev sem OTP — `usu_verificacao=1`, `per_habilitado=1` de imediato |
| `POST /api/admin/usuarios/:usu_id/redefinir-senha` | Dev | Redefine senha de Admin/Dev sem email, invalida sessões ativas |
| `GET /api/admin/stats/documentos` | Admin/Dev | Totais de documentos por tipo e status OCR |
| `GET /api/admin/stats/contratos` | Dev | Resumo de contratos: ativos, expirados, sem contrato, vencendo em 90 dias |
| `PATCH /api/admin/usuarios/:usu_id/status` | Admin/Dev | Ativa/inativa conta sem penalidade |
| `GET /api/admin/matriculas` | Admin/Dev | Lista matrículas com usuário, curso e escola |
| `GET /api/admin/avaliacoes` | Admin/Dev | Lista avaliações com participantes (escopo por avaliado) |
| `GET /api/admin/veiculos` | Admin/Dev | Lista veículos com dados do proprietário |
| `GET /api/admin/logs/exportar` | Dev | Exporta AUDIT_LOG como CSV (máx. 10.000 registros) |
| `GET /api/caronas/buscar` | JWT | Busca com `?car_status=`, `?data=`, `?esc_id=`, `?cur_id=` |

#### Sistema de contratos de escolas — v11

Novo campo na tabela `ESCOLAS`:

| Coluna | Tipo | Descrição |
|--------|------|-----------|
| `esc_contrato_duracao` | `ENUM('1ano','2anos','5anos')` | Duração do contrato (NULL = sem contrato) |
| `esc_contrato_inicio` | `DATE` | Data de início |
| `esc_contrato_expira` | `DATE` | Expiração calculada no backend (inicio + duração) |

**Rotas de contrato:**
- `POST /api/admin/escolas/:esc_id/contrato` — define/renova contrato. Body: `{ duracao, data_inicio? }`.
- `DELETE /api/admin/escolas/:esc_id/contrato` — cancela contrato (todos os campos → NULL).

**Migration v11** para bancos existentes (comentada em `insert.sql`):
```sql
ALTER TABLE ESCOLAS
...
#### Controle de acesso por contrato expirado

Quando o contrato de uma escola expira, o acesso é bloqueado automaticamente:

| Papel | Lógica |
|-------|--------|
| **Administrador** (per_tipo=1) | Bloqueado se o contrato da sua escola (`per_escola_id`) expirou |
| **Usuário comum** (per_tipo=0) | Bloqueado se o domínio do email (`@xxx.edu.br`) pertence a escola com contrato expirado |
| **Desenvolvedor** (per_tipo=2) | Nunca bloqueado (gerencia os contratos) |

**Pontos de verificação:**
- `POST /api/usuarios/login` — bloqueia novos logins com 403 imediatamente
- `POST /api/usuarios/refresh` — bloqueia renovação de tokens; invalida o refresh token, forçando novo login após renovação do contrato

Implementado em `verificarContratoEscola()` em `src/utils/authHelper.js`. A função degrada graciosamente se a migration v11 ainda não foi aplicada ao banco.
