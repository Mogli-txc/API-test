...
function iniciarAutoCloseCaronas() {
    ...
    cron.schedule('0 0 * * *', async () => {
        try {
            ...
            // PASSO 5: Notifica motoristas — aviso de sistema sobre o encerramento
            for (const m of motoristas) {
                notificar({
                    usu_id:   m.motorista_id,
                    tipo:     TIPOS.SISTEMA,
                    titulo:   'Carona encerrada automaticamente',
                    mensagem: 'Sua carona foi encerrada automaticamente. Confira o histórico se precisar de informações.',
